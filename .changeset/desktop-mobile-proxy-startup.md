...
Load the proxy opener via a lazy `import()` (injected into
`MobileGatewayManager`) so the E2E stack stays out of the startup path and loads
only when the user enables mobile — in a `ulid`-free lazy chunk, post
`app.whenReady`.
