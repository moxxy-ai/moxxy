...
it('validates PE architecture, protocol and exact executable digest before launch', () => {
  ...
  const manifest = { protocolVersion: 1, architecture: 'x64', sha256: createHash('sha256').update(bytes).digest('hex') };
  expect(() => validateHelperArtifact(bytes, manifest)).not.toThrow();
  expect(() => validateHelperArtifact(bytes, { ...manifest, protocolVersion: 2 })).toThrow();
