...
import { MAX_FRAME_BYTES, PROTOCOL_VERSION, responseSchema } from './contracts.js';
...
export class HelperTransport {
  ...
  private send(method: string, params: unknown, signal: AbortSignal): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => this.fail(new Error('Computer Use timed out; action not retried. Observe again.')), this.timeoutMs);
      signal.addEventListener('abort', abort, { once: true });
      this.pending = { id, resolve, reject, dispose: () => {
        clearTimeout(timer);
