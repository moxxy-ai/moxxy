...
void wait_for_access(HWND window, bool needs_focus) {
  ...
  if (!user_paused && (!needs_focus || has_target_focus(window))) return;
  ...
  for (;;) {
    ...
    if (!user_paused && (!needs_focus || has_target_focus(window))) break;
    auto state=user_paused ? ControlState::paused_by_user : ControlState::waiting_for_focus;
