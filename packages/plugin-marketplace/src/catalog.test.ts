...
const entry = DEFAULT_MARKETPLACE_CATALOG[0];

describe('resolveMarketplaceEntry / resolveMarketplacePackageName', () => {
  it('resolves by id and by package name', () => {
    expect(resolveMarketplaceEntry(entry.id)?.packageName).toBe(entry.packageName);
    expect(resolveMarketplaceEntry(entry.packageName)?.id).toBe(entry.id);
  });
  it('returns the target verbatim when unknown', () => {
    expect(resolveMarketplaceEntry('nope')).toBeUndefined();
    expect(resolveMarketplacePackageName('@scope/unknown')).toBe('@scope/unknown');
  });
});
...
describe('formatMarketplaceStatus / buildMarketplaceActionOptions', () => {
  const installed = new Set([entry.packageName]);
  const empty = new Set<string>();

  it('reports install status', () => {
    expect(formatMarketplaceStatus(entry, empty, empty)).toContain('not installed');
    expect(formatMarketplaceStatus(entry, installed, empty)).toContain('installed');
    expect(formatMarketplaceStatus(entry, installed, installed)).toBe('disabled');
  });

  it('offers install when absent, and open/disable/remove when an installed UI plugin', () => {
    const absent = buildMarketplaceActionOptions({ entry, installedPackageNames: empty, disabledPackageNames: empty });
    expect(absent.map((o) => o.value)).toEqual(['install', 'back']);

    const present = buildMarketplaceActionOptions({ entry, installedPackageNames: installed, disabledPackageNames: empty });
