...
describe('Expo Go compatibility', () => {
  ...
  it('uses a CommonJS Metro config extension when the mobile package is ESM', async () => {
    ...
    await expect(access(join(process.cwd(), 'mobile', 'metro.config.cjs'), constants.F_OK)).resolves.toBeUndefined();
    await expect(access(join(process.cwd(), 'mobile', 'metro.config.js'), constants.F_OK)).rejects.toThrow();
  });

  it('loads Metro config from the installed Expo package', async () => {
    const metroConfig = await readFile(join(process.cwd(), 'mobile', 'metro.config.cjs'), 'utf8');
