...
export function SchedulerList({
  ...
  return (
    <View className="gap-3">
      ...
      {error ? (
        <View className="rounded-card border border-red/20 bg-red/10 p-4">
          <Text className="text-[14px] font-black text-red">Scheduler error</Text>
          <Text className="mt-1 text-[13px] leading-5 text-red">{error}</Text>
        </View>
      ) : null}

      {schedules.length === 0 ? (
        <View className="rounded-card border border-cardBorder bg-cardBg p-5 shadow-card" style={{ shadowOpacity: 0.08 }}>
          <Text className="text-[16px] font-black text-text">No schedules visible</Text>
      ...
      {schedules.map((schedule) => (
        ...
        >
          <View className="flex-row items-start gap-3">
            <View className={`mt-1.5 h-2.5 w-2.5 rounded-pill ${schedule.enabled ? 'bg-green' : 'bg-cardBorderStrong'}`} />
            <View className="min-w-0 flex-1">
              <View className="flex-row items-start justify-between gap-3">
                <View className="min-w-0 flex-1">
                  <Text className="text-[16px] font-black leading-6 text-text">{schedule.name}</Text>
                  {schedule.promptPreview ? (
                    <Text className="mt-1 text-[13px] leading-5 text-muted" numberOfLines={3}>
                      {schedule.promptPreview}
                    </Text>
                  ) : null}
                </View>
              ...
              {schedule.lastError ? (
                <Text className="mt-3 text-[12px] leading-5 text-red" numberOfLines={3}>
                  {schedule.lastError}
                </Text>
              ) : null}
            </View>
          ...
          <View className="mt-4 flex-row gap-2">
            <Pressable
              className={`min-h-11 flex-1 flex-row items-center justify-center gap-2 rounded-pill px-4 ${
            ...
            >
              ...
              />
              <Text className={`text-[13px] font-black ${schedule.enabled ? 'text-muted' : 'text-white'}`}>
                {schedule.enabled ? 'Pause' : 'Enable'}
              </Text>
            </Pressable>
            <Pressable
              className="min-h-11 flex-row items-center justify-center gap-2 rounded-pill border border-red/20 px-4"
              onPress={() => onDelete(schedule)}
              accessibilityRole="button"
            >
              <MobileIcon name="x" size={16} strokeWidth={2.5} color="#ef4444" />
              <Text className="text-[13px] font-black text-red">Delete</Text>
            </Pressable>
          </View>
        </View>
      ))}
...
function StatusPill({ enabled, label }: { readonly enabled: boolean; readonly label: string }) {
  return (
    <View className={`rounded-pill px-3 py-1 ${enabled ? 'bg-green/10' : 'bg-appBg'}`}>
      <Text className={`text-[11px] font-black ${enabled ? 'text-green' : 'text-muted'}`}>{label}</Text>
    </View>
  );
}
...
function Badge({ label, tone }: { readonly label: string; readonly tone: 'danger' | 'muted' }) {
  return (
    <View className={`rounded-pill px-3 py-1 ${tone === 'danger' ? 'bg-red/10' : 'bg-appBg'}`}>
      <Text className={`text-[11px] font-black ${tone === 'danger' ? 'text-red' : 'text-muted'}`}>{label}</Text>
    </View>
  );
}
