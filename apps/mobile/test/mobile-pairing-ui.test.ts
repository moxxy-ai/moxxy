...
import { buildPairingUiState, maskPairingCode } from '../src/pairingUi';
...
describe('mobile pairing UI model', () => {
  ...
  it('masks the pairing code (bearer token) for on-screen display', () => {
    expect(maskPairingCode('')).toBe('------');
    const masked = maskPairingCode('abcd1234ef567890');
    expect(masked).toBe('abcd••••••••');
    expect(masked).not.toContain('1234');
    expect(maskPairingCode('xy')).toBe('••');
  });
});
