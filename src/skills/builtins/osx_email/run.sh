...
# --- THREAT SCANNER ---
# Returns threat count and details via global variables
THREAT_COUNT=0
THREAT_DETAILS=""
...
do_fetch() {
    ...
    # AppleScript to fetch email metadata and content from Mail.app
    local script
...
APPLESCRIPT_TEMPLATE
    ...
    local raw_output
    raw_output=$(printf '%s' "$result" | jq -r '.stdout // ""')
    ...
    # Parse and save each email as a file
    local today
    today=$(date +%Y-%m-%d)
    local idx=0
    local metadata_json="["
    ...
    while IFS= read -r line; do
        ...
        if [ "$line" = "===EMAIL_END===" ]; then
            if [ "$in_email" -eq 1 ] && [ -n "$current_from" ]; then
                ...
                # Append metadata (no body content!)
                if [ "$idx" -gt 1 ]; then
                    metadata_json="${metadata_json},"
                fi
                metadata_json="${metadata_json}{\"index\":${idx},\"from\":$(jq -n --arg v "$current_from" '$v'),\"subject\":$(jq -n --arg v "$current_subject" '$v'),\"date\":$(jq -n --arg v "$current_date" '$v'),\"filename\":$(jq -n --arg v "$filename" '$v')}"
            fi
            in_email=0
            in_body=0
            continue
...
do_list() {
    ...
    for f in "$EMAILS_DIR"/*.txt; do
        ...
        files_json="${files_json}{\"filename\":$(jq -n --arg v "$basename" '$v'),\"from\":$(jq -n --arg v "$from" '$v'),\"subject\":$(jq -n --arg v "$subject" '$v'),\"date\":$(jq -n --arg v "$date" '$v')}"
    done
    files_json="${files_json}]"
...
do_permissive_read() {
    ...
    if [ "$dry_run" = "--dry-run" ]; then
        # Dry-run: report threats only, no content
        local verdict="SAFE"
        if [ "$THREAT_COUNT" -gt 0 ]; then
            verdict="UNSAFE"
        fi
        local details_escaped
        details_escaped=$(printf '%b' "$THREAT_DETAILS" | jq -Rs '.')
...
"
                ...
                # Strip HTML tags from safe lines
                local clean_line
...
"
        ...
        details_escaped=$(printf '%b' "$THREAT_DETAILS" | jq -Rs '.')
        local content_escaped
        content_escaped=$(printf '%s' "$redacted" | jq -Rs '.')
